...
var CurrencyAmount = class _CurrencyAmount extends Fraction {
  ...
  constructor(currency, numerator, denominator) {
    ...
    invariant2(JSBI3.lessThanOrEqual(this.quotient, MaxUint256), "AMOUNT");
    this.currency = currency;
    this.decimalScale = JSBI3.exponentiate(JSBI3.BigInt(10), JSBI3.BigInt(currency.decimals));
  }
  add(other) {
...
var ONE_HUNDRED = new Fraction(JSBI4.BigInt(100));
...
var Price = class _Price extends Fraction {
   ...
   */
  multiply(other) {
    invariant3(this.quoteCurrency.equals(other.baseCurrency), "TOKEN");
  ...
  quote(currencyAmount) {
    invariant3(currencyAmount.currency.equals(this.baseCurrency), "TOKEN");
...
var BaseCurrency = class {
  ...
  constructor(chainId, decimals, symbol, name) {
    invariant4(Number.isSafeInteger(chainId), "CHAIN_ID");
    invariant4(decimals >= 0 && decimals < 255 && Number.isInteger(decimals), "DECIMALS");
...
// src/utils/validateAndParseAddress.ts
...
var Token = class extends BaseCurrency {
  ...
  constructor(chainId, address, decimals, symbol, name, bypassChecksum, buyFeeBps, sellFeeBps) {
    ...
    if (buyFeeBps) {
      invariant5(buyFeeBps.gte(BigNumber.from(0)), "NON-NEGATIVE FOT FEES");
    }
    if (sellFeeBps) {
      invariant5(sellFeeBps.gte(BigNumber.from(0)), "NON-NEGATIVE FOT FEES");
    }
  ...
  sortsBefore(other) {
    ...
    invariant5(this.address.toLowerCase() !== other.address.toLowerCase(), "ADDRESSES");
...
// src/entities/ether.ts
var Ether = class _Ether extends NativeCurrency {
  constructor(chainId) {
    super(chainId, 18, "ETH", "Ether");
  }
  get wrapped() {
    const weth9 = WETH9[this.chainId];
    invariant6(!!weth9, "WRAPPED");
    return weth9;
  }
  static _etherCache = {};
...
// src/utils/computeZksyncCreate2Address.ts
...
// src/utils/sortedInsert.ts
import invariant7 from "tiny-invariant";
function sortedInsert(items, add, maxSize, comparator) {
  invariant7(maxSize > 0, "MAX_SIZE_ZERO");
  invariant7(items.length <= maxSize, "ITEMS_SIZE");
  if (items.length === 0) {
    items.push(add);
    return null;
  } else {
...
var MAX_SAFE_INTEGER = JSBI6.BigInt(Number.MAX_SAFE_INTEGER);
var ZERO = JSBI6.BigInt(0);
var ONE = JSBI6.BigInt(1);
var TWO = JSBI6.BigInt(2);
function sqrt(value) {
  invariant8(JSBI6.greaterThanOrEqual(value, ZERO), "NEGATIVE");
